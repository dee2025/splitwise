...
| `[x]` | Home | `/api/groups`, `/api/expenses` | Show greeting, balance cards, overall balance, top groups, recent expenses. Change any `/expenses` navigation to group/activity navigation. |
| `[x]` | Groups list | `/api/groups`, `/api/expenses` | Search groups and members, admin/member filters, group balance, group image, open group detail. |
...
| `[x]` | Group Summary | `/api/groups/:groupId`, `/api/expenses?groupId=` | Show description, total expenses, members count, group type, currency, invite link for admins, and balance summary. |
| `[x]` | Group Members | `/api/groups/:groupId/members`, `/api/groups/:groupId`, `/api/users/search` | Add registered user by search, add custom member, remove member, enforce admin-only controls. |
| `[x]` | Group settings | `PUT /api/groups/:groupId`, `POST /api/groups/:groupId/invite/regenerate`, `POST /api/uploads/image` | Edit group fields, upload image, regenerate invite link, delete group. |
| `[x]` | Notifications | `/api/notifications` | Load notifications, filter all/read/unread, mark one read, mark all read, delete selected. |
| `[x]` | Profile overview | `/api/users/profile` | Show avatar, name, username, email, group count, expense count. |
| `[x]` | Edit profile | `PUT /api/users/profile`, `POST /api/uploads/image` | Update full name, username, contact, bio, avatar, and refresh auth user state. |
| `[x]` | Change password | `PUT /api/users/change-password` | Validate current and new password inputs, submit, show errors. |
| `[x]` | Delete account | `DELETE /api/users/account` | Require confirmation, optional password for local accounts, logout after deletion. |
| `[x]` | Logout | `/api/auth/logout` | Call backend logout, clear secure token, sign out Google account, return to login. |
...
3. `[x]` Update Home so no UI navigates to `/expenses`.
...
| Status | Check |
| --- | --- |
| `[x]` | `flutter pub get` succeeds inside `mobile`. |
| `[x]` | `dart format lib test` succeeds inside `mobile`. |
| `[x]` | `flutter analyze` succeeds inside `mobile`. |
| `[x]` | `flutter test` succeeds inside `mobile`. |
